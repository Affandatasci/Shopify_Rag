...
- **Embed prefix on queries only**: `bge-small-en-v1.5`'s model card asks for the `"Represent this sentence for searching relevant passages: "` prefix on search queries. It's applied in `search_policies` at query time and must **never** be applied to documents at ingest time (`ingest.py` embeds raw chunk text).
- **Small embedding model, on purpose**: originally used `mxbai-embed-large-v1` (335M params, ~1.3GB) to match Forge Physique, but that pushed Streamlit Community Cloud's free-tier RAM over its limit and caused repeated crash-restarts in production. Switched to `bge-small-en-v1.5` (33M params, ~130MB) — same prefix convention, same normalized-cosine setup, just lighter. If you ever change the embedding model again: update `EMBED_MODEL_NAME` **and** `EMBED_DIM` in `ingest.py` (it deletes and recreates the Qdrant collection at that dimension every run) and `EMBED_MODEL_NAME` in `agent.py`, then re-run `ingest.py` before redeploying — a dimension mismatch between the collection and the query model fails loudly.
